    ...
    public LinkedList<String> getPlaces(String Price, String Addres, String Caracteristics ,String Categorie)
    {
   	 try ( Session session = driver.session() )
        {
   		 ...
   		 LinkedList<String> places = session.readTransaction( new TransactionWork<LinkedList<String>>()
            {
                ...
                public LinkedList<String> execute( Transaction tx )
                {
                    Result result = tx.run( "MATCH (p:place) WHERE p.department='" + Addres + "' AND p.cost='" + Price + "' AND p.relation='" + Categorie +"' AND p.caracteristic='" + Caracteristics + "' RETURN p.name,p.department,p.cost,p.relation,p.caracteristic order by p.score desc");
   ...
   public LinkedList<String> getAPlaces(String Price, String Addres, String Caracteristics ,String Categorie)
    {
   	 try ( Session session = driver.session() )
        {
   		 ...
   		 LinkedList<String> places = session.readTransaction( new TransactionWork<LinkedList<String>>()
            {
                ...
                public LinkedList<String> execute( Transaction tx )
                {
                    Result result = tx.run( "MATCH (p:place) WHERE (p.department='" + Addres + "' AND p.cost='" + Price + "' AND p.relation='" + Categorie +"' AND NOT p.caracteristic='" + Caracteristics + "') OR (p.department='" + Addres + "' AND p.cost='" + Price + "' AND p.caracteristic='" + Caracteristics + "' AND NOT p.relation='" + Categorie +"' ) OR (p.department='" + Addres + "' AND p.caracteristic='" + Caracteristics + "' AND p.relation='" + Categorie +"' AND NOT p.cost='" + Price + "') OR (p.caracteristic='" + Caracteristics + "' AND p.cost='" + Price + "' AND p.relation='" + Categorie +"' AND NOT p.department='" + Addres + "') RETURN p.name,p.department,p.cost,p.relation,p.caracteristic order by p.score desc");
                    LinkedList<String> myplaces = new LinkedList<String>();
                    List<Record> registros = result.list();
                    for (int i = 0; i < registros.size(); i++) {
                   	 //myactors.add(registros.get(i).toString());
